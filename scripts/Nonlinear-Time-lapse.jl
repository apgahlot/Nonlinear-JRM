...
if isfile(datadir(joinpath(sim_name,"dmonitor_nrep_"*savename(data_dict, "jld2"; digits=6))))
    ...
    data_dict = @strdict nsrc nrec nv nv_id snr timeD dtD f0 space_order d_monitor qm
    @tagsave(
...
c2 = Vector{SetIntersectionProjection.set_definitions}();
c3 = Vector{SetIntersectionProjection.set_definitions}();
...
# Bound constraints for innovations difference
m_min     = -.05;
m_max     = .05;
set_type  = "bounds";
TD_OP     = "identity";
app_mode  = ("matrix","");
custom_TD_OP = ([],false);
push!(c2, set_definitions(set_type, TD_OP, m_min, m_max, app_mode, custom_TD_OP));

# TV  constraint for innovations and innovation difference
...
push!(c1, set_definitions(set_type, TD_OP, m_min, m_max, app_mode, custom_TD_OP));
push!(c2, set_definitions(set_type, TD_OP, m_min, m_max, app_mode, custom_TD_OP));
push!(c3, set_definitions(set_type, TD_OP, m_min, m_max, app_mode, custom_TD_OP));


# # TV  constraint for innovations difference
# TV = get_TD_operator(model_baseline, "TV", Float32)[1];
# m_min     = 0.0;
# m_max     = norm(TV*vec(m_baseline - m_monitor), 1); # Lets cheat for now and put the true TV
# set_type  = "l1";
...
(P2, TD2, set2) = setup_constraints(c2, model_baseline, Float32);
(P3, TD3, set3) = setup_constraints(c3, model_baseline, Float32);
...
(TD2, AtA2, l2, y2) = PARSDMM_precompute_distribute(TD2, set2, model_baseline, options);
(TD3, AtA3, l3, y3) = PARSDMM_precompute_distribute(TD3, set3, model_baseline, options);
...
Pr2 = x -> PARSDMM(x, AtA2, TD2, set2, P2, model_baseline, options)
Pr3 = x -> PARSDMM(x, AtA3, TD3, set3, P3, model_baseline, options)
...
if inv_type == "joint"
    ...
    @tagsave(
        datadir(joinpath(sim_name, "sol_nrep_justatest_"*savename(data_dict, "jld2"; digits=6))),
        data_dict;
        safe=true)
    ...
    subplot(413)
    plot_simage(reshape(splitv(sol_jrm.x)[3]-splitv(sol_jrm.x)[2], n)', d; new_fig=false, vmax = 0.004, name="Inverted time-lapse (nJRM)", cmap="cet_CET_D1", cbar=true, perc=99.9)
    subplot(414)
    plot_simage(m_monitor' - m_baseline', d; new_fig=false, name="True time-lapse", vmax = 0.004, cmap="cet_CET_D1", cbar=true, perc=99)
    tight_layout()

    fig_name = @strdict nsrc nrec nv nv_id snr timeD dtD f0 niter batchsize inv_type space_order
    safesave(joinpath(plot_path, "sol_nrep_justatest_"*savename(fig_name; digits=6)*".png"), fig);
    ...
    data_dict = @strdict nsrc nrec nv nv_id snr timeD dtD f0 niter batchsize inv_type space_order sol_base sol_mon1
    @tagsave(
        datadir(joinpath(sim_name, "sol_nrep_justatest_"*savename(data_dict, "jld2"; digits=6))),
        data_dict;
        safe=true)
    ...
    subplot(413)
    plot_simage(reshape(sol_mon1.x.-sol_base.x, n)', d; new_fig=false, vmax = 0.004, name="Inverted time-lapse (Ind)", cmap="cet_CET_D1", cbar=true, perc=99.9)
    subplot(414)
    plot_simage(m_monitor' - m_baseline', d; new_fig=false, vmax = 0.004, name="True time-lapse", cmap="cet_CET_D1", cbar=true, perc=99)
    tight_layout()
