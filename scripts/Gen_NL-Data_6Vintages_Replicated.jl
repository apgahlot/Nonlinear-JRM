...
xsrc = convertToCell(range(start=25f0, stop=(n[1]-1)*d[1]-30f0, length=nsrc))
...
srcGeometry_m1 = Geometry(xsrc, ysrc, zsrc; dt=dtD, t=timeD)
srcGeometry_m2 = Geometry(xsrc, ysrc, zsrc; dt=dtD, t=timeD)
srcGeometry_m3 = Geometry(xsrc, ysrc, zsrc; dt=dtD, t=timeD)
srcGeometry_m4 = Geometry(xsrc, ysrc, zsrc; dt=dtD, t=timeD)
srcGeometry_m5 = Geometry(xsrc, ysrc, zsrc; dt=dtD, t=timeD)
...
for i=1:5
    ...
    sxm = xsrc[sxi][1]
    subplot(3,5,i)
    plot_sdata(d_baseline.data[sxi]; cmap="PuOr", new_fig=false, name="Baseline sx=$(sxb)m", cbar=true)
    subplot(3,5,i+5)
    plot_sdata(d_monitor1.data[sxi]; cmap="PuOr", new_fig=false, name="Monitor1 sx=$(sxm)m", cbar=true)
    subplot(3,5,i+10)
    plot_sdata(d_baseline.data[sxi] - d_monitor1.data[sxi]; cmap="PuOr", new_fig=false, name="Time-lapse sx=$(sxb)m", cbar=true)
end
tight_layout()

fig_name = @strdict nsrc nrec nv nv_id snr timeD dtD f0 space_order
safesave(joinpath(plot_path, "Shot_record_rep"*savename(fig_name; digits=6)*".png"), fig)
