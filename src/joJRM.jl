...
function joTransitionJRM(L::Integer, nn::Integer; DDT=Float32, name::String="joTransitionJRM")
    # turn the JRM components at L-1 to L -- assume gamma changes from sqrt(L-1) to sqrt(L)
    return joKron(joMatrix(DDT.(vcat(diagm(vcat(DDT(sqrt(L)/sqrt(L-1)), ones(DDT,L-1))), vcat(zeros(DDT, L-1), DDT(1))'))), joEye(nn; DDT=DDT))
